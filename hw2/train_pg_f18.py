...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        nn_layer = input_placeholder
        for i in range(n_layers):
            nn_layer = tf.layers.dense(nn_layer, size, activation, name='hidden_layer_{}'.format(i))
        output_layer = tf.layers.dense(nn_layer, output_size, output_activation, name='output_layer')
    return output_layer
...
class Agent(object):
    ...
    def build_computation_graph(self):
        ...
        if self.nn_baseline:
            ...
            self.baseline_prediction = tf.squeeze(build_mlp(
                                    ...
                                    size=self.size))
            # YOUR_CODE_HERE
            self.sy_target_n = tf.placeholder(shape=[None], name="sy_target", dtype=tf.float32)
    ...
    def sum_of_rewards(self, re_n):
        ...
        q_n = []
        for re_p in re_n:
            idxes = np.arange(len(re_p))
            gammas = np.power(self.gamma, idxes)
            if self.reward_to_go:
                len_p_disc = len(re_p)
                re_p_disc = [np.sum(gammas[:(len_p_disc-t)] * re_p[t:]) for t in idxes]
            else:
                re_p_disc = [np.sum(gammas * re_p) for t in idxes]
            q_n += re_p_disc
        return np.float32(q_n)
    ...
    def estimate_return(self, ob_no, re_n):
        ...
        if self.normalize_advantages:
            ...
            #raise NotImplementedError
            EPSILON = 1e-8
            adv_n = (adv_n - np.mean(adv_n)) / (np.std(adv_n) + EPSILON) # YOUR_CODE_HERE
        return q_n, adv_n
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            #raise NotImplementedError
            EPSILON = 1e-8
            target_n = (q_n - np.mean(q_n)) / (np.std(q_n) + EPSILON)
